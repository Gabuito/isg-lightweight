 ...
 */
export async function searchGoogleImage(query, config) {
    ...
    try {
        ...
        try {
            const cookieButton = await page.waitForSelector('button[aria-label="Aceitar todos"], button[aria-label="Accept all"]', { timeout: options.timeout });
            if (cookieButton)
        ...
        const thumbnails = await page.$$('#search g-img[class][style]');
