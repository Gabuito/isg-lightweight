...
import type { ImageConfig } from './index.ts';

  export async function saveImageLocally(imageUrl: string , query: string, options? : ImageConfig): Promise<void> {
  try {
    const settings = {
    ...
    await sharp(image)
    .toFormat(settings.format as keyof FormatEnum, { quality: settings.quality }) // Você pode ajustar a qualidade (0-100)
